const CombatSystem = {
    ...
    loadAssets() {
        ...
        if (window.state && window.state.selectedNft && window.state.selectedNft.image) {
            playerImg.src = window.state.selectedNft.image;
        } else {
            playerImg.src = 'https://ipfs.io/ipfs/QmZ8nyVf4XvK6mZqLpC3P1pXN3zYJ5GzW1R1vX7p6N3zYJ';
        }
        playerImg.onload = () => { this.state.playerImage = playerImg; };

        const enemyImg = new Image();
        enemyImg.crossOrigin = "anonymous";
        enemyImg.src = 'https://ipfs.io/ipfs/QmXyN3zYJ5GzW1R1vX7p6N3zYJ5GzW1R1vX7p6N3zYJ';
        enemyImg.onload = () => { this.state.enemyImage = enemyImg; };
    },
    ...
    setupControls() {
        ...
        const handleInteraction = (btn, keyCode, isStart) => {
            if (!btn) return;
        ...
        this.state.bullets.forEach((b, bi) => {
            this.state.enemies.forEach((e, ei) => {
                if (b.x < e.x + e.w && b.x + b.w > e.x && b.y < e.y + e.h && b.y + b.h > e.y) {
                    ...
                    this.spawnExplosion(e.x + e.w/2, e.y + e.h/2, '#ff3b6b');
    ...
    draw() {
        ...
        // Enemies
        this.state.enemies.forEach(e => {
            if (this.state.enemyImage) {
                ctx.drawImage(this.state.enemyImage, e.x, e.y, e.w, e.h);
            } else {
